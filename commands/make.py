...
class make(Command):
    ...
    def run(self, data):
        ...
        # convert the readme into html and put it in the public folder
        elif len(data) == 1 and data[0] == 'readme':
            with open("public/index.html", 'w') as output_file:
                output = Markdown(extras=['break-on-newline']).convert(open("readme.md").read())
                output = "<style> code { white-space: pre; } </style>\n" + output
                ...
                try:
                    ...
                    for (_, c) in clsmembers:
                        if issubclass(c, Model) & (c is not Model):
                            model = c()
                            table_names.append({ 'name': model.table, 'indexes': model.indexes })
                except Exception as e:
                    pass
        return table_names
